...
- **The Completed count no longer stops at `MAX_JOB_HISTORY`.** Finish more than
  ...
  the list silently disagree: *"Showing the 500 most recent of 1,153."* Raise
  `MAX_JOB_HISTORY` to keep more of them listed. **Clear** still zeroes
  everything, evicted tally included. `Show metadata jobs` is honoured in the
  new totals, so pruned background scans don't inflate the badges while their
  rows stay hidden.
